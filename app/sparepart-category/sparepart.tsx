...
export default function SparePart() {
  ...
  const fetchProductsByCategory = async () => {
    try {
      setLoading(true);
      const response = await getProductsByCategory(warehouse, 4, page, 20);
      if (response?.data) {
  ...
  return (
    <>
      ...
      >
        <Text style={{ fontSize: 20, fontWeight: "bold" }}>Spare Parts</Text>
        <Text style={{ fontSize: 14, paddingBottom: 10, marginBottom: 10 }}>
          Components to repair or maintain production
        </Text>
        <FlatList
